...
/**
 * StepUpload — Step 1 for the Site View pipeline.
 *
 * LEFT:  Site / plot images (up to 4, mandatory)
 * RIGHT: Avatar / presenter (prebuilt RE agents, upload, or my-assets)
 */
export function StepUpload({
  ...
  const [draggingSite, setDraggingSite] = useState(false);
  ...
  // ── Site images ─────────────────────────────────────────────────────────────
  const handleSiteFiles = useCallback(
    (files) => {
      const validFiles = Array.from(files).filter((f) => f.type.startsWith("image/"));
      if (validFiles.length === 0) return toast.error("Please upload image files.");
      if (siteImages.length + validFiles.length > 4) {
        return toast.error("Maximum 4 site images allowed.");
      }
      ...
      setSiteImages((prev) => [...prev, ...withPreview]);
    },
  ...
  const onDropSite = (e) => {
    e.preventDefault();
    setDraggingSite(false);
    handleSiteFiles(e.dataTransfer.files);
  };
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="text-center space-y-1">
        <h2 className="text-2xl font-bold font-heading tracking-tight">
          Upload Site Photos & Presenter
        </h2>
        <p className="text-sm text-neutral-500">
          Add development site shots and select or upload a presenter for your site-tour video
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          ...
          <div
            onDragOver={(e) => { e.preventDefault(); setDraggingSite(true); }}
            onDragLeave={() => setDraggingSite(false)}
            onDrop={onDropSite}
            ...
            {siteImages.length === 0 ? (
              <label className="flex flex-col items-center gap-3 py-10 cursor-pointer">
                ...
                <div className="text-center">
                  <p className="text-sm font-medium">Drop site photos here or click to upload</p>
                ...
                <input
                  ...
                  accept="image/*"
                  className="hidden"
                  onChange={(e) => handleSiteFiles(e.target.files)}
                />
              </label>
            ) : (
          ...
          {siteImages.length > 0 && (
            <div className="flex items-center gap-2 text-[11px] text-emerald-600 dark:text-emerald-400">
              ...
              <span>
                {siteImages.length} photo{siteImages.length !== 1 ? "s" : ""} added
              </span>
            </div>
          )}
          ...
          <div className="relative overflow-hidden rounded-2xl border border-neutral-200 bg-white p-4">
            ...
            <div className="pl-3 flex items-center gap-3">
              ...
              <p className="text-xs text-neutral-600 leading-relaxed">
                Upload photos of the development site — entrance gate, open plots, infrastructure
                (roads, poles), surrounding area, boundary walls, or any on-site signage. More
                context = better video.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground">Choose from RE Agents or use your own</p>
            </div>
          </div>
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const res = await fetch("/api/site-view/presenter/upload", {
                          method: "POST",
                          body: fd,
                        });
