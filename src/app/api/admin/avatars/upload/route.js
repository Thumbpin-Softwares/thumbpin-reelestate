    ...
    for (const file of files) {
      if (!ALLOWED_TYPES.has(file.type)) {
        return NextResponse.json(
          { error: `Only PNG, JPG, WEBP images are allowed. Invalid file: ${file.name}` },
          { status: 400 }
        );
      }
    }
...
export async function GET(request) {
  ...
  try {
    // List all objects in Avatars folder
    ...
    // Return all collections - Group by collection-id
    ...
    for (const obj of objects) {
      ...
      try {
        ...
        if (collId && !collectionsMap.has(collId)) {
          // Get the first file (lowest index) for cover image
          const collectionObjects = objects.filter(async (o) => {
            try {
              const m = await s3.send(new HeadObjectCommand({ Bucket: BUCKET, Key: o.Key }));
              return m.Metadata?.['collection-id'] === collId;
            } catch {
              return false;
            }
          });
          ...
          console.log(`Added collection: ${collId} - ${metadata.Metadata?.['collection-name']}`);
        }
      } catch (err) {
