...
export async function POST(request) {
  ...
  try {
    ...
    await dbConnect();
    const existingJob = await NewsAnchorJob.findOne({ jobId }).lean();
    if (existingJob) {
      return NextResponse.json(
    ...
    console.log(
      `[NewsAnchor] Resolved ${avatarUrls.length} avatar URL(s):`,
      avatarUrls,
    );
    ...
    const creditResult = await consumeCreditsForAction({
      ...
      metadata: { endpoint: "/api/news-anchor/generate-pipeline" },
    });
    if (!creditResult.ok) {
    ...
    await NewsAnchorJob.create({ jobId, userId, status: "running" });
    ...
    await Promise.all(
      locationBufs.map(async (buf, i) => {
        try {
          ...
          console.warn(
            `[NewsAnchor] Failed to upload location image ${i}:`,
            e.message,
          );
        }
      }),
    ...
    if (locationBufs.length > 0 && locationR2Urls.filter(Boolean).length === 0) {
      console.error(
        `[NewsAnchor] All ${locationBufs.length} location image upload(s) failed — walkthrough will have no image reference.`,
      );
    }
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        // by polling GET /api/news-anchor/jobs/:jobId instead of re-POSTing
        // (which would double-bill credits and double-fire fal/Seedance generations).
        async function persistJob(patch) {
          try {
            await NewsAnchorJob.updateOne({ jobId }, { $set: patch });
          } catch (e) {
            console.error("[NewsAnchor] Job persist failed:", e.message);
          }
        }
...
${script}`;
          ...
          try {
            ...
            console.warn(
              "[NewsAnchor] LLM split failed, using word-count split:",
              splitErr.message,
            );
          }
          ...
          console.log(
            `[NewsAnchor] Durations — intro:${seedanceDuration}s wt:${walkthroughDuration}s cta:${ctaDuration}s`,
          );
...
${part3_cta}`;
                ...
                if (match) {
                  ...
                  if (allValid) {
                    ...
                    console.warn(
                      "[NewsAnchor] Roman→native conversion incomplete — keeping all parts in Roman for consistency:",
                      parsed,
                    );
                  }
                }
                ...
                console.warn(
                  "[NewsAnchor] Roman→native conversion failed:",
                  adaptErr.message,
                );
              }
...
${part3_cta}`;
                ...
                console.warn(
                  "[NewsAnchor] Native→Roman transliteration failed:",
                  romanErr.message,
                );
              }
            }
          ...
          console.log(
            `[NewsAnchor] Intro image_urls (${introInput.image_urls?.length ?? 0}):`,
            introInput.image_urls,
          );
          console.log(
            `[NewsAnchor] Walkthrough image_urls (${walkthroughInput.image_urls?.length ?? 0}):`,
          ...
          await Promise.allSettled([
            callSeedanceAndUpload(introInput, userId, "nanchor-avatar")
              .then((url) => {
                avatarVideoUrl = url;
                console.log("[NewsAnchor] Intro avatar uploaded:", url);
                send({
                  type: "seedance_done",
                  avatarVideoUrl: url,
              ...
              .catch((err) => {
                console.error(
                  "[NewsAnchor] Intro Seedance failed:",
                  err.message,
                );
                send({
                  type: "seedance_error",
            ...
            callSeedanceAndUpload(walkthroughInput, userId, "nanchor-walkthrough")
              .then((url) => {
                walkthroughVideoUrl = url;
                console.log("[NewsAnchor] Walkthrough uploaded:", url);
                send({
              ...
              .catch((err) => {
                console.error(
                  "[NewsAnchor] Walkthrough Seedance failed:",
                  err.message,
                );
                send({
                  type: "seedance_error",
            ...
            callSeedanceAndUpload(ctaInput, userId, "nanchor-cta")
              .then((url) => {
                ctaVideoUrl = url;
                console.log("[NewsAnchor] CTA avatar uploaded:", url);
                send({
              ...
              .catch((err) => {
                console.error(
                  "[NewsAnchor] CTA Seedance failed:",
                  err.message,
                );
                send({
                  type: "seedance_error",
          ...
          if (primaryUrl) {
            try {
              ...
              await Asset.create({
                userId,
                name: `News Anchor — ${new Date().toLocaleDateString()}`,
                url: primaryUrl,
                type: "clip",
                metadata: {
                  source: "news-anchor",
              ...
              console.error("[NewsAnchor] DB save error:", dbErr);
            }
          }
          ...
          clearInterval(pingInterval);
          console.error("[NewsAnchor] Pipeline error:", err);
          await persistJob({
            status: "error",
          ...
          if (userId && debit) {
            await refundCreditsForAction({
              ...
              debit,
              metadata: {
                endpoint: "/api/news-anchor/generate-pipeline",
    ...
    console.error("[NewsAnchor] Outer error:", error);
