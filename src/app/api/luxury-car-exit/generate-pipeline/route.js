...
/**
 * POST /api/luxury-car-exit/generate-pipeline
 *
...
export async function POST(request) {
  ...
  try {
    ...
    const creditResult = await consumeCreditsForAction({
      ...
      metadata: { endpoint: "/api/luxury-car-exit/generate-pipeline" },
    });
    if (!creditResult.ok) {
...
${part2}`;
          ...
          if (primaryUrl) {
            try {
              ...
              await Asset.create({
                userId,
                name: `Luxury Car Exit — ${new Date().toLocaleDateString()}`,
                url: primaryUrl,
                type: "clip",
                metadata: {
          ...
          if (userId && debit) {
            await refundCreditsForAction({
              ...
              debit,
              metadata: {
                endpoint: "/api/luxury-car-exit/generate-pipeline",
