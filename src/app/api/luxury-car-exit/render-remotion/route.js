...
export async function POST(request) {
  ...
  (async () => {
    try {
      ...
      const outputPath = join(tmpdir(), `luxury-car-exit-${Date.now()}.mp4`);
      ...
      try {
        ...
        await Asset.create({
          userId,
          name: `Luxury Car Exit (exported) — ${new Date().toLocaleDateString()}`,
          url,
          type: "video",
          metadata: { source: "luxury-car-exit-export", exportedFrom: "luxury-car-exit" },
        });
      } catch (dbErr) {
        console.error("[luxury-car-exit render-remotion] DB save error:", dbErr);
      }
