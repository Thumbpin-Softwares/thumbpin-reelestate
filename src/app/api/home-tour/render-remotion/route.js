  ...
  (async () => {
    try {
      ...
      try {
        ...
        await Asset.create({
          userId,
          name: `Home Tour (exported) — ${new Date().toLocaleDateString()}`,
          url,
          type: "video",
          metadata: { source: "home-tour-export", exportedFrom: "home-tour" },
        });
      } catch (dbErr) {
        console.error("[home-tour render-remotion] DB save error:", dbErr);
      }
